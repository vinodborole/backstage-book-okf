...
resource: https://backstage.io/docs/next/auth
timestamp: '2026-08-03T09:44:12.848210+00:00'
---

# Authentication in Backstage

This documentation is written for [the new frontend system](/docs/next/frontend-system/). If you are on the old frontend system you may want to read [its own article](/docs/next/auth/index--old) instead.
...
Identity management and the Sign-In page in Backstage will only block external access when using the new backend system, without setting `backend.auth.dangerouslyDisableDefaultAuthPolicy` in configuration. Even so, the frontend bundle is not protected from external access, protecting it requires the use of the [experimental public entry point](/docs/next/tutorials/enable-public-entry). You can learn more about this in the [Threat Model](/docs/next/overview/threat-model#operator-responsibilities).

## Built-in Authentication Providers

Backstage comes with many common authentication providers in the core library:

- [Auth0](/docs/next/auth/auth0/provider)
- [Atlassian](/docs/next/auth/atlassian/provider)
- [Azure](/docs/next/auth/microsoft/provider)
- [Azure Easy Auth](/docs/next/auth/microsoft/easy-auth)
- [Bitbucket](/docs/next/auth/bitbucket/provider)
- [Bitbucket Server](/docs/next/auth/bitbucketServer/provider)
- [Cloudflare Access](/docs/next/auth/cloudflare/provider)
- [GitHub](/docs/next/auth/github/provider)
- [GitLab](/docs/next/auth/gitlab/provider)
- [Google](/docs/next/auth/google/provider)
- [Google IAP](/docs/next/auth/google/gcp-iap-auth)
- [Keycloak](/docs/next/auth/keycloak/provider) (community-maintained)
- [Okta](/docs/next/auth/okta/provider)
- [OAuth 2 Custom Proxy](/docs/next/auth/oauth2-proxy/provider)
- [OneLogin](/docs/next/auth/onelogin/provider)
- [OpenShift](/docs/next/auth/openshift/provider)
- [VMware Cloud](/docs/next/auth/vmware-cloud/provider)
...
Using an authentication provider for sign-in is something you need to configure both in the frontend app as well as the `auth` backend plugin. For information on how to configure the backend app, see [Sign-in Identities and Resolvers](/docs/next/auth/identity-resolver). The rest of this section will focus on how to configure sign-in for the frontend app.
...
Some auth providers are so-called "proxy" providers, meaning they're meant to be used behind an authentication proxy. Examples of these are [Amazon Application Load Balancer](https://github.com/backstage/backstage/blob/master/contrib/docs/tutorials/aws-alb-aad-oidc-auth.md), [Azure EasyAuth](/docs/next/auth/microsoft/easy-auth), [Cloudflare Access](/docs/next/auth/cloudflare/provider), [Google Identity-Aware Proxy](/docs/next/auth/google/gcp-iap-auth) and [OAuth2 Proxy](/docs/next/auth/oauth2-proxy/provider).
...
Backstage uses [Passport](http://www.passportjs.org/) under the hood, which has a wide library of authentication strategies for different providers. See [Add authentication provider](/docs/next/auth/add-auth-provider) for details on adding a new Passport-supported authentication method.
...
1. Source page: https://backstage.io/docs/next/auth
