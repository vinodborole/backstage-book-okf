...
- *Processors* , that validate, analyze, and mutate the raw entity data into its
final form.

The high level processes involved are:

- *Ingestion* , where entity providers fetch raw entity data from external
sources and seed it into the database,
- *Processing* , where the policies and processors continually treat the ingested
data and may emit both other raw entities (that are also subject to
processing), errors, relations to other entities, etc.,
- *Stitching* , where all of the data emitted by various processors are assembled
...
graph. This is *not* the same thing as relations! The purpose of these edges,
...
relations emitted by *other* entity processing steps that happen to point at
the current entity
...
the child entity.
- The child entity is *not* removed from the catalog, but stays around until
explicitly deleted via the catalog API, implicitly if`orphanStrategy: delete` configuration is set (the default), or until it is "reclaimed" by the original
...
be read successfully, does *not* lead to orphaning. Hard errors, including the
