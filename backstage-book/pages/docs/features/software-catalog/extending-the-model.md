...
- There are a number of builtin versioned *kinds* , such as`Component` ,`User` etc. These encapsulate the high level concept of an entity, and define the
...
in the interface.
- Entities may have a number of *[annotations](/docs/features/software-catalog/well-known-annotations)* on
...
- Entities may have a number of *relations* , expressing how they relate to each
...
a `pagerduty.com` prefix for pagerduty related annotations, but maybe not`ldap.com` for LDAP annotations since it's not directly affiliated with or
...
prefixed with a domain, in a way that makes sense to a reader and connects it
clearly to the system (or the maker of the system). For example, you might use
a `pagerduty.com` prefix for pagerduty related labels, but maybe not`ldap.com` for LDAP labels since it's not directly affiliated with or owned by
