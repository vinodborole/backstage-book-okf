---
type: Web Page
title: Running Backstage behind a Corporate Proxy | Backstage Software Catalog and
  Developer Platform
description: Guide on how to configure Backstage to work behind a corporate proxy
  using Node.js built-in proxy support.
resource: https://backstage.io/docs/tutorials/corporate-proxy
...
Per [ADR014](/docs/architecture-decisions/adrs-adr014), Backstage backend code should use native `fetch()`, which works with Node.js's proxy out of the box. Some core packages and many [community plugins](https://github.com/backstage/community-plugins/) still use `node-fetch` (see [ADR013](/docs/architecture-decisions/adrs-adr013)) or `cross-fetch` (for isomorphic packages). Both libraries delegate to `node:http`/`node:https` internally and do **not** set a custom HTTP agent by default, which means Node.js's proxy works for them as well.
...
1. Source page: https://backstage.io/docs/tutorials/corporate-proxy
