---
type: Web Page
title: Setting up CI | Backstage Software Catalog and Developer Platform
description: Configure continuous integration checks for your Backstage instance.
resource: https://backstage.io/docs/getting-started/ci
...
A Backstage instance is a living project. As you add plugins, customize configuration, and update dependencies, things can break in subtle ways: a TypeScript error in one package, a config typo that prevents the backend from starting, or a Docker image that no longer builds. Continuous Integration (CI) catches these problems on every pull request, before they reach production.
...
These checks apply regardless of which CI system you use. Most map to commands provided by the Backstage CLI.
...
[repo lint](/docs/tooling/cli/module-lint#repo-lint) for available options.
...
configuration. See
[repo test](/docs/tooling/cli/module-test#repo-test) for available
options.
...
[repo list-deprecations](/docs/tooling/cli/module-maintenance#repo-list-deprecations)
...
not surface. See
[repo build](/docs/tooling/cli/module-build#repo-build) for available
options.
...
default. See
[config:check](/docs/tooling/cli/module-config#configcheck) for available
options.
...
This step is optional. Docker builds can be slow, so some teams prefer to run them only on merges to the default branch or on a scheduled basis rather than on every pull request.
...
Tests run before the full build to give faster feedback on the most common failure mode. Build runs before the Docker build because the Dockerfile expects pre-built backend bundles.
...
- **Docker-in-Docker** : Some CI systems require extra configuration to run`docker build` inside a pipeline. Check your platform's documentation
for Docker support.
- **Environment variables** : Set`CI=true` and`NODE_OPTIONS=--max-old-space-size=8192` in your pipeline environment.
...
plugins. This setting prevents out-of-memory crashes during`yarn tsc:full` and`yarn build:all` .

# Citations

1. Source page: https://backstage.io/docs/getting-started/ci
