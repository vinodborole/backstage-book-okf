...
read the [old frontend system version of this guide](/docs/getting-started/configure-app-with-plugins--old)
instead.
...
[created a Backstage app](/docs/getting-started/) and want to add an existing plugin
to it.
...
With the new frontend system, plugins are automatically discovered and installed in your app through [feature discovery](/docs/frontend-system/building-apps/installing-plugins#feature-discovery). This is enabled by the default `app-config.yaml` setting:
  ...
  extensions:
    - page:tech-radar:
        config:
          path: /tech-radar
```
For details on what configuration options are available, refer to the plugin's own documentation or the [Configuring Extensions](/docs/frontend-system/building-apps/configuring-extensions) guide.

### 4. Optional: Manually install a plugin

If you need more control over plugin installation, or if [feature discovery](/docs/frontend-system/building-apps/installing-plugins#feature-discovery) is not enabled, you can install plugins manually by importing them and passing them to `createApp`:
...
```
For more details and alternative installation methods, see [Installing Plugins](/docs/frontend-system/building-apps/installing-plugins).
...
If you want to customize sidebar behavior — for example, reordering items, grouping them, or adding custom entries — you can override the built-in `app/nav` extension. See the [migration guide's sidebar section](/docs/frontend-system/building-apps/migrating#app-root-sidebar) for details on how to create a custom sidebar layout.

# Citations

1. Source page: https://backstage.io/docs/getting-started/configure-app-with-plugins
