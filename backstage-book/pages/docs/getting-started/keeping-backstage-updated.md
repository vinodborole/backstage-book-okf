...
When a given dependency version is the *same* between different packages, the
dependency is hoisted to the main `node_modules` folder in the monorepo root to
be shared between packages. When *different* versions of the same dependency are
...
1. Source page: https://backstage.io/docs/getting-started/keeping-backstage-updated
