...
Classification of an [entity](#entity) in the Backstage Software Catalog, for example *service*, *database*, or *team*. An element of the [kind|namespace|name triplet](#kind-namespace-name-triplet) that is an important concept for uniqueness.
...
Another name for [Software Templates](#software-templates). (The term comes from the use of Software Templates as *scaffolds* for building new components and projects.)
...
A purpose for which a [user role](#user-role) interacts with Backstage. Related to [Objective](#objective): An objective is *what* a user wants to accomplish; a use case is *how* the user does it.
