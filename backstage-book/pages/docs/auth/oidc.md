---
type: Web Page
title: OIDC provider from scratch | Backstage Software Catalog and Developer Platform
description: This section shows how to enable and use the Backstage OIDC provider.
resource: https://backstage.io/docs/auth/oidc
...
read the [old frontend system version of this guide](/docs/auth/oidc--old)
instead.
...
2. Configure the provider in `app-config.yaml` .
3. Configure a sign-in resolver.
4. Add the provider to the Backstage sign-in page.
...
- `additionalScopes` : Requests additional scopes on top of the default`openid profile email` scopes. Do not configure`scope` directly, as the OIDC provider will reject
      ...
      development:
        ...
        signIn:
          resolvers:
            - resolver: emailMatchingUserEntityProfileEmail
```
If none of the built-in resolvers are suitable, you can write a custom resolver. See [Building Custom Resolvers](/docs/auth/identity-resolver#building-custom-resolvers) for details.
...
```
For a more detailed explanation about resolvers, see the [Identity Resolver](/docs/auth/identity-resolver) page.
...
For more information about sign-in configuration, see [Sign-in Configuration](/docs/auth/#sign-in-configuration).

# Citations

1. Source page: https://backstage.io/docs/auth/oidc
